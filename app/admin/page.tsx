...
type Tab = "projects" | "blog" | "requests";

export default function AdminDashboardPage() {
  const [activeTab, setActiveTab] = useState<Tab>("projects");

  const tabs = [
    { id: "projects" as Tab, label: "Projects", icon: "🚀", color: "from-accent-purple to-accent-blue" },
    { id: "blog" as Tab, label: "Blog Posts", icon: "📝", color: "from-accent-blue to-accent-cyan" },
    { id: "requests" as Tab, label: "Requests", icon: "📧", color: "from-accent-cyan to-accent-yellow" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="glass-strong rounded-2xl border border-white/20 p-2 flex gap-2"
      >
        {tabs.map((tab) => (
          <motion.button
            key={tab.id}
